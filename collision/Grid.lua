	...
	for i = 1, n do
		repeat
			...
			if
				...
				or body.bounds.min.x > world.bounds.max.x
				or body.bounds.max.y < world.bounds.min.y
				or body.bounds.min.y > world.bounds.max.y
			then
			...
			if not body.region or newRegion.id ~= body.region.id or forceUpdate then
				...
				for col = union.startCol, union.endCol do
					for row = union.startRow, union.endRow do
						...
						local isInsideNewRegion = (
							col >= newRegion.startCol
							and col <= newRegion.endCol
							and row >= newRegion.startRow
							and row <= newRegion.endRow
						)

						local isInsideOldRegion = (
							col >= body.region.startCol
							and col <= body.region.endCol
							and row >= body.region.startRow
							and row <= body.region.endRow
						)

						-- remove from old region buckets

						if not isInsideNewRegion and isInsideOldRegion and isInsideOldRegion and bucket then
