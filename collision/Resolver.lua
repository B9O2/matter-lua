...
function Resolver.solvePosition(pairs, timeScale)
	...
	local pair, collision, bodyA, bodyB, normal, bodyBtoAX, bodyBtoAY, contactShare, positionImpulse, contactCount = {}
	...
	for i = 1, n do
		repeat
			...
			bx, by =
				vectorAdd(bodyB.positionImpulse.x, bodyB.positionImpulse.y, bodyB.position.x, bodyB.position.y, tempA)
			sx, sy =
				vectorSub(bodyB.position.x, bodyB.position.y, collision.penetration.x, collision.penetration.y, tempB)
...
function Resolver.preSolveVelocity(pairs)
	...
	local pair, contacts, collision, bodyA, bodyB, normal, tangent, contact, contactVertex, normalImpulse, tangentImpulse, offsetX, offsetY
		...
		repeat
			...
			for j = 1, c do
				...
				if normalImpulse ~= 0 or tangentImpulse ~= 0 then
					...
					if not (bodyA.isStatic or bodyA.isSleeping) then
						offsetX, offsetY =
							vectorSub(contactVertex.x, contactVertex.y, bodyA.position.x, bodyA.position.y, tempA)
					...
					if not (bodyB.isStatic or bodyB.isSleeping) then
						offsetX, offsetY =
							vectorSub(contactVertex.x, contactVertex.y, bodyB.position.x, bodyB.position.y, tempA)
	...
	for i = 1, n do
		repeat
			...
			for j = 1, c do
				...
				-- offsetA
				offsetAX, offsetAY =
					vectorSub(contactVertexX, contactVertexY, bodyA.position.x, bodyA.position.y, tempA)
				-- offsetB
				offsetBX, offsetBY =
					vectorSub(contactVertexX, contactVertexY, bodyB.position.x, bodyB.position.y, tempB)
				...
				relativeVelocityX, relativeVelocityY =
					vectorSub(velocityPointAX, velocityPointAY, velocityPointBX, velocityPointBY, tempE)
				...
				if
					normalVelocity < 0
					and normalVelocity * normalVelocity > Resolver._restingThresh * timeScaleSquared
				then
				...
				if tangentVelocity * tangentVelocity > Resolver._restingThreshTangent * timeScaleSquared then
					...
					contact.tangentImpulse =
						Common.clamp(contact.tangentImpulse + tangentImpulse, -maxFriction, maxFriction)
	...
	for i = 1, n do
		repeat
			...
			for j = 1, c do
				...
				local velocityPointA =
					Vector.add(bodyA.velocity, Vector.mult(Vector.perp(offsetA), bodyA.angularVelocity), tempC)
				local velocityPointB =
					Vector.add(bodyB.velocity, Vector.mult(Vector.perp(offsetB), bodyB.angularVelocity), tempD)
				...
				local normalForce = Common.clamp(pair.separation + normalVelocity, 0, 1)
