...
function Bounds.update(bounds, vertices, velocity)
	...
	for i = 1, n do
		...
		if vertex.x > bounds.max.x then
			bounds.max.x = vertex.x
		end
		if vertex.x < bounds.min.x then
			bounds.min.x = vertex.x
		end
		if vertex.y > bounds.max.y then
			bounds.max.y = vertex.y
		end
		if vertex.y < bounds.min.y then
...
function Bounds.contains(bounds, point)
	return (point.x >= bounds.min.x and point.x <= bounds.max.x and point.y >= bounds.min.y and point.y <= bounds.max.y)
end
...
function Bounds.overlaps(boundsA, boundsB)
	return (
		...
		and boundsA.max.x >= boundsB.min.x
		and boundsA.max.y >= boundsB.min.y
		and boundsA.min.y <= boundsB.max.y
	)
end
