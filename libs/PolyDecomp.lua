...
function PolyDecomp.polygonMakeCCW(polygon)
	...
	for i = 2, #polygon do
		if v[i][2] < v[br][2] or (v[i][2] == v[br][2] and v[i][1] > v[br][1]) then
			br = i
		end
	end
	...
	if
		not PolyDecomp.isLeft(
			PolyDecomp.polygonAt(polygon, br - 1),
			PolyDecomp.polygonAt(polygon, br),
			PolyDecomp.polygonAt(polygon, br + 1)
		)
	then
...
function PolyDecomp.polygonCanSee2(polygon, a, b)
	...
	for i = 1, n do
		repeat
			...
			if
				PolyDecomp.lineSegmentsIntersect(
					PolyDecomp.polygonAt(polygon, a),
					PolyDecomp.polygonAt(polygon, b),
					PolyDecomp.polygonAt(polygon, i),
					PolyDecomp.polygonAt(polygon, i + 1)
...
function PolyDecomp.polygonSlice(polygon, cutEdges)
	...
	if
		type(cutEdges) == "table"
		and #cutEdges
		and type(cutEdges[1]) == "table"
		and #cutEdges[1] == 2
		and type(cutEdges[1][1]) == "table"
	then
		local polys = { polygon }
	...
	for i = 1, #polygon do
		if PolyDecomp.polygonIsReflex(poly, i) then
			...
			for j = 1, #polygon do
				...
				if
					PolyDecomp.isLeft(
						PolyDecomp.polygonAt(poly, i - 1),
						PolyDecomp.polygonAt(poly, i),
						PolyDecomp.polygonAt(poly, j)
					)
					and PolyDecomp.isRightOn(
						PolyDecomp.polygonAt(poly, i - 1),
						PolyDecomp.polygonAt(poly, i),
						PolyDecomp.polygonAt(poly, j - 1)
					)
				then
					p = PolyDecomp.getIntersectionPoint(
						PolyDecomp.polygonAt(poly, i - 1),
						PolyDecomp.polygonAt(poly, i),
						PolyDecomp.polygonAt(poly, j),
						PolyDecomp.polygonAt(poly, j - 1)
