...
function SAT.collides_SRC(bodyA, bodyB, previousCollision)
	local overlapAB, overlapBA, minOverlap, collision, canReusePrevCol = false

	if previousCollision then
		-- estimate total motion
		local parentA = bodyA.parent
		local parentB = bodyB.parent
		local motion = parentA.speed * parentA.speed
...
function SAT.collides(bodyA, bodyB, previousCollision)
	...
	local overlapAB, overlapBA, minOverlap, collision
	...
	if
		(minOverlapAxisX * (bodyB.position.x - bodyA.position.x))
			+ (minOverlapAxisY * (bodyB.position.y - bodyA.position.y))
		< 0
	then
		collision.normal = {
			x = minOverlapAxisX,
...
function SAT.collides_notoptimised(bodyA, bodyB, previousCollision)
	...
	local overlapAB, overlapBA, minOverlap, minOverlapAxisX, minOverlapAxisY, collision
...
function SAT._overlapAxes(verticesA, verticesB, axes)
	...
	local axis, axisX, axisY, overlap
	...
	return resultAxisNumber and { overlap = resultOverlap, axis = resultAxis, axisNumber = resultAxisNumber }
		or { overlap = resultOverlap }
end
...
function SAT._findSupports(bodyA, bodyB, normalX, normalY)
	...
	local distance, vertex, vertexA, vertexB, vertexX, vertexY
...
function SAT._findSupports_SRC(bodyA, bodyB, normal)
	...
	local distance, vertex, vertexA, vertexB
...
function SAT._findSupports__(bodyA, bodyB, normalX, normalY)
	...
	local distance, vertex, vertexA, vertexB, vertexX, vertexY, vertexAIndex
