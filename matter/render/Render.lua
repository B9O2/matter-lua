...
local function _getPixelRatio(canvas)
	...
	local backingStorePixelRatio = context.webkitBackingStorePixelRatio
	...
	for i = 1, n do
		repeat
			...
			for k = p > 1 and 2 or 1, p do
				...
				vertices:push(part.vertices[1].x):push(part.vertices[1].y)

				local v = #part.vertices
				for j = 1, v do
					vertices:push(part.vertices[j].x):push(part.vertices[j].y)

					if part.vertices[j].isInternal and not showInternalEdges then
						local index = (j + 1) % #part.vertices
						index = index ~= 0 and index or #part.vertices

						vertices:push(part.vertices[index].x):push(part.vertices[index].y)
	...
	for i = 1, n do
		...
		for k = p > 1 and 2 or 1, p do
			...
			for j = 1, v do
				gfx.drawText(
					i .. "-" .. j,
					part.position.x + (part.vertices[j].x - part.position.x) * 0.95,
					part.position.y + (part.vertices[j].y - part.position.y) * 0.95
				)
			end
		end
	end
...
function Render.mousePosition(render, mouse, context) end
...
function Render.bodyAxes(render, bodies, context)
	local engine = render.engine
	local options = render.options
	local part, body, parts, vertices, p
...
function Render.collisions(render, pairs, context)
	...
	local pair, collision, corrected, bodyA, bodyB
		...
		repeat
			...
			if #pair.activeContacts > 0 then
				...
				if collision.bodyB == collision.supports[1].body or collision.bodyA.isStatic then
					vertices:push(normalPosX - collision.normal.x * 8):push(normalPosY - collision.normal.y * 8)
				else
					vertices:push(normalPosX + collision.normal.x * 8):push(normalPosY + collision.normal.y * 8)
				end

				vertices:push(normalPosX):push(normalPosY)
			end
...
function Render.grid(render, grid, context)
	...
	for i = 1, n do
		repeat
			...
			gfx.drawRect(
				0.5 + tonumber(region[1]) * bucketWidth,
